...
void make_command_and_add_to_action(struct action* act, int nargs, char **args) {
	struct command *cmd;
    int (*func)(int nargs, char **args);
    int kw, n;
    if (nargs == 0) {
	...
	ERROR("*******************************add %s success", args[0]);
}
void add_command(const char *trigger, int nargs, char **args) {
		struct listnode *node;
		struct action *act;
		list_for_each(node, &action_list) {
			act = node_to_item(node, struct action, alist);
			if (!strcmp(act->name, trigger)) {
				//find the trigger action, so add the command here
				make_command_and_add_to_action(act, nargs, args);
				break;
			}
		}
}
