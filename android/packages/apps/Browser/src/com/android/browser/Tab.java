...
class Tab implements PictureListener {
    ...
    private final WebChromeClient mWebChromeClient = new WebChromeClient() {
		...
		private void acquireWakeLock() {
		 if (mFullScreenWakeLock == null) {
			 PowerManager pm = (PowerManager) mContext.getSystemService(Context.POWER_SERVICE);
			 mFullScreenWakeLock = pm.newWakeLock(PowerManager.SCREEN_BRIGHT_WAKE_LOCK, "flash_plugin");
			 mFullScreenWakeLock.acquire();
			 Log.d(LOGTAG, "#########################acquire");
		 }
	 }
		
	 private void releaseWakeLock(){
		 if(mFullScreenWakeLock != null && mFullScreenWakeLock.isHeld()){
			 mFullScreenWakeLock.release();
			 mFullScreenWakeLock = null;
	...
	private boolean newTabFlag = false;

	public boolean isNewTab() {
		return newTabFlag;
	}

	public void setNotNewTab() {
		newTabFlag = false;
	}
