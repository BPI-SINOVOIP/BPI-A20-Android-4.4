...
static struct sw_device_info gsensors[] = {        
        ...
        {   "stk831x", {0x3d, 0x22            }, 0x12, {0x28, 0x3c      }, 0},
...
static struct sw_device_info ctps[] = {
	...
	{ "aw5306_ts",{0x38, 0x39},   0x01, {0xa8                    }, 0},
	{ "ilitek_ts",{      0x41},   0x00, {0x00                    }, 0},
};
...
static int sw_analytic_write_info(char * src_string, struct sw_device *sw)
{
        ...
        while((src_string[i++]) && (i <= strlen(src_string))) {  
...
{
        int i = 0;
        while((info[number].id_value[i]) && (i <= REG_VALUE_NUMBER)) {
...
static void sw_detect_gsensor(void)
{
        int ret = -1;
        int device_number = 0;
...
static void sw_detect_ctp(void)
{
        int ret = -1;
        int device_number = 0;
