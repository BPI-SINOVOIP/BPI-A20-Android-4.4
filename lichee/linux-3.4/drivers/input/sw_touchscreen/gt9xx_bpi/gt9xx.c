...
static int ctp_detect(struct i2c_client *client, struct i2c_board_info *info)
{
    ...
    if(twi_id == adapter->nr){
    	GTP_INFO("%s: addr = %x", __func__, client->addr);
        ret = i2c_test(client);
        if(!ret){
        	GTP_ERROR("%s:I2C connection might be something wrong", __func__);
        	return -ENODEV;
        }else{           	    
...
void gtp_gpio_as_input(u32 gpio)
{
	int gpio_status = -1;
...
void gtp_gpio_as_output(u32 gpio, int value)
{
	int gpio_status = -1;
...
void gtp_gpio_as_int(u32 gpio)
{
    ...
    if (sw_gpio_eint_setall_range(&cfg, 1)) {
        GTP_ERROR("%s: gpio %d eint set all range failed\n", __func__, gpio);
        return;
    }
}

void gtp_gpio_free(u32 gpio)
{
	gpio_free(gpio);
}
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
    ...
    gtp_gpio_as_output(GTP_RST_GPIO, 0);   // begin select I2C slave addr
    msleep(ms);                         // T2: > 10ms
    // HIGH: 0x28/0x29, LOW: 0xBA/0xBB
    gtp_gpio_as_output(GTP_INT_GPIO, client->addr == 0x14);

    msleep(2);                          // T3: > 100us
    gtp_gpio_as_output(GTP_RST_GPIO, 1);
...
static s8 gtp_wakeup_sleep(struct goodix_ts_data * ts)
{
    ...
    if (CHIP_TYPE_GT9F == ts->chip_type)
    {
        ...
        gtp_gpio_as_output(GTP_INT_GPIO, 1);
        msleep(5);
...
static s8 gtp_request_irq(struct goodix_ts_data *ts)
{
	...
	int_handle = sw_gpio_irq_request(GTP_INT_GPIO,CTP_IRQ_MODE,(peint_handle)goodix_ts_irq_handler,ts);
	//int_handle = 0;
	if (!int_handle) {
		GTP_ERROR( "goodix_probe: request irq failed\n");
...
{   
		...
		printk("[BPI-TP]%s:cfgname:%s\n",__func__,cfgname);
		printk("[BPI-TP]%s:cfg_index:%d\n",__func__,cfg_index);
		...
		twi_id = config_info.twi_id;
		if(twi_id == 0){
        	GTP_ERROR("%s:read config error!",__func__);
            return 0;
        }

		GTP_INFO("%s, twi_id = %d", __func__, twi_id);
...
static int __devinit goodix_ts_init(void)
{
	...
	if (input_fetch_sysconfig_para(&(config_info.input_type))) {
		GTP_ERROR("%s: ctp_fetch_sysconfig_para err", __func__);
		return 0;
	}
	else {
		ret = input_init_platform_resource(&(config_info.input_type));
		if (0 != ret) {
			GTP_ERROR("%s:ctp_ops.init_platform_resource err", __func__);    
		}
	}
